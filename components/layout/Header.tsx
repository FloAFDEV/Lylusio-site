...
export const Header = () => {
	...
	return (
		<>
					...
					>
						{mainLinks.map((link) => (
							<Link
								...
								href={link.href}
								onClick={handleNavClick}
								className="font-medium text-[15px] text-foreground/80 hover:text-foreground motion-safe:transition-all duration-200 ease-out relative group/link focus:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 rounded-md px-1 hover:-translate-y-[1px]"
							>
								<MenuLabel label={link.label} />
								<span className="absolute -bottom-1 left-0 w-0 h-[3px] bg-gradient-to-r from-accent via-gold to-accent opacity-0 group-hover/link:w-full group-hover/link:opacity-100 motion-safe:transition-all duration-300 ease-out shadow-[0_0_8px_hsl(var(--gold)/0.6)]" />
							</Link>
						))}
					</nav>
			...
			<div
				id="mobile-menu"
				className={`xl:hidden fixed top-0 right-0 h-full w-3/4 max-w-xs bg-card shadow-lg border-l border-accent/20 z-50 motion-safe:transition-transform duration-400 ease-in-out ${
							...
							link.hasSubmenu ? (
								...
								>
									...
									>
										<MenuLabel label={link.label} />
										<ChevronDown
											className={`w-4 h-4 motion-safe:transition-transform duration-300 ease-in-out ${
												mobileSubmenuOpen
													? "rotate-180"
													: ""
											}`}
									...
									{/* Sous-menu mobile smooth */}
									<div
										id={`mobile-submenu-${link.label
											.toLowerCase()
											.replace(/\s+/g, "-")}`}
										aria-hidden={!mobileSubmenuOpen}
										className={`overflow-hidden motion-safe:transition-all duration-350 ease-in-out ${
											mobileSubmenuOpen
									...
									>
										<div className="pl-4 py-2 space-y-1 border-l-2 border-accent/30 ml-2">
											{link.subItems?.map(
												(item, subIndex) => (
													<Link
														...
														href={item.href}
														onClick={handleNavClick}
														className="block font-medium text-sm text-foreground/80 hover:text-accent hover:bg-accent/5 rounded-lg px-3 py-2 motion-safe:transition-all duration-250 ease-out min-h-[44px] flex items-center focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
														style={{
															opacity:
																mobileSubmenuOpen
																	? 1
											...
											{/* Lien direct “Voir tous les accompagnements” mobile */}
											{mobileSubmenuOpen && (
												<Link
													href={link.href}
													onClick={handleNavClick}
													className="block text-sm text-muted-foreground hover:text-accent py-2 pl-2 motion-safe:transition-colors duration-200 ease-out focus:outline-none focus-visible:ring-2 focus-visible:ring-accent rounded-lg"
													style={{
														opacity:
															mobileSubmenuOpen
																? 1
								...
								<Link
									...
									href={link.href}
									onClick={handleNavClick}
									className="flex items-center font-medium text-foreground/80 hover:text-accent py-3 motion-safe:transition-all duration-300 ease-out min-h-[44px] focus:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 rounded-md"
									style={{
										opacity: isMobileOpen ? 1 : 0,
										transform: isMobileOpen
											? "translateX(0)"
